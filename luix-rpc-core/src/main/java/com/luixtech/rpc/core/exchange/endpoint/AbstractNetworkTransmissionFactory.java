...
public abstract class AbstractNetworkTransmissionFactory implements NetworkTransmissionFactory {
    ...
    public Server createServer(Url providerUrl, InvocationHandleable handler) {
        ...
        synchronized (ADDRESS_2_SHARED_SERVER) {
            ...
            boolean sharedChannel = providerUrl.getBooleanOption(ProtocolConstants.SHARED_SERVER, ProtocolConstants.SHARED_SERVER_VAL_DEFAULT);
            if (!sharedChannel) {
    ...
    public void destroyServer(Server server, Url providerUrl) {
        boolean shareChannel = providerUrl.getBooleanOption(ProtocolConstants.SHARED_SERVER, ProtocolConstants.SHARED_SERVER_VAL_DEFAULT);
        if (!shareChannel) {
            server.close();
            return;
        }
