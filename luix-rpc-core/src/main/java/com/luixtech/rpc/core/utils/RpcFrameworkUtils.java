...
import com.luixtech.rpc.core.switcher.impl.SwitcherHolder;
...
public class RpcFrameworkUtils {
    ...
    public static String getProviderKey(Url providerUrl) {
        return providerUrl.getProtocol() + Url.PROTOCOL_SEPARATOR + providerUrl.getAddress() + DIR_SEPARATOR_UNIX
    ...
    public static String getMethodKey(String protocol, String interfaceName, String methodName, String form, String version) {
        ...
        sb.append(protocol).append(Url.PROTOCOL_SEPARATOR);
    ...
    public static boolean checkIfCanShareServiceChannel(Url source, Url target) {
        ...
        if (!StringUtils.equals(source.getOption(ProtocolConstants.CODEC), target.getOption(ProtocolConstants.CODEC))) {
            return false;
        }
...
//        }
        ...
        if (!StringUtils.equals(source.getOption(ProtocolConstants.MAX_CONTENT_LENGTH), target.getOption(ProtocolConstants.MAX_CONTENT_LENGTH))) {
            return false;
        }

        if (!StringUtils.equals(source.getOption(ProtocolConstants.MAX_SERVER_CONN), target.getOption(ProtocolConstants.MAX_SERVER_CONN))) {
            return false;
        }

        if (!StringUtils.equals(source.getOption(ProtocolConstants.MAX_THREAD), target.getOption(ProtocolConstants.MAX_THREAD))) {
            return false;
        }

        if (!StringUtils.equals(source.getOption(ProtocolConstants.WORK_QUEUE_SIZE), target.getOption(ProtocolConstants.WORK_QUEUE_SIZE))) {
            return false;
        }

        return StringUtils.equals(source.getOption(ProviderConstants.HEALTH_CHECKER), target.getOption(ProviderConstants.HEALTH_CHECKER));
    }
    ...
    public static boolean checkIfCanShallClientChannel(Url source, Url target) {
        ...
        if (!StringUtils.equals(source.getOption(ProtocolConstants.SERIALIZER), target.getOption(ProtocolConstants.SERIALIZER))) {
            return false;
        }
