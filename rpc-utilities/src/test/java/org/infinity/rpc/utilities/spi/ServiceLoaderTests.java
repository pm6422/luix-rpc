...
public class ServiceLoaderTests extends TestCase {
    ...
    public void testSingletonInitialization() {
        // 单例模式下只会构造一次实例
        Assert.assertEquals(1, ServiceLoader.forClass(SpiSingletonInterface.class).load("spitest").spiHello());
        Assert.assertEquals(1, ServiceLoader.forClass(SpiSingletonInterface.class).load("spitest").spiHello());
    }
    ...
    public void testPrototypeInitialization() {
        // 多例模式下在每次获取的时候进行实例化
        Assert.assertEquals(1, ServiceLoader.forClass(SpiPrototypeInterface.class).load("spiPrototypeTest").spiHello());
        Assert.assertEquals(2, ServiceLoader.forClass(SpiPrototypeInterface.class).load("spiPrototypeTest").spiHello());
    }
}
